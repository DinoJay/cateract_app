...
function d3TimeSwitch(yDate) {
  ...
  case (d3.timeWeek.count(...yDate.domain()) <= 4):
    return {
      intervalKey: 'weeks',
...
function update() {
  ...
  const maxRad = d3.max(nestedData, d => d.sumRadiation);
  ...
  d3.select('.rad-legend').append('line')
  ...
  (function updateRadLegend() {
    const stepNum = 5;
    const tickVals = d3.range(0, maxRad, maxRad / stepNum);
    const radAxis = d3.axisTop(radBarWidth)
                      .tickValues(tickVals.length > 0 ? tickVals : [0])
                      .tickSize(self.dim.legendHeight)
                      .tickPadding(-self.dim.legendHeight / 2);
    ...
    d3.select('.rad-axis')
      ...
      .attr('dx', () => (radBarWidth(maxRad / stepNum) - radBarWidth(0)) / 2);
